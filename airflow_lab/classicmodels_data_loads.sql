...
alter table orders 
add column `order_date` date after `orderDate`,
add column `required_date` date after `requiredDate`,
add column `shipped_date` date after `shippedDate`;

update orders set order_date=date_add(orderDate, INTERVAL 18 YEAR);
update orders set required_date=date_add(requiredDate, INTERVAL 18 YEAR);
update orders set shipped_date=date_add(shippedDate, INTERVAL 18 YEAR);
...
add column `payment_date` date after `paymentDate`;

update payments set payment_date=date_add(paymentDate, INTERVAL 18 YEAR);
